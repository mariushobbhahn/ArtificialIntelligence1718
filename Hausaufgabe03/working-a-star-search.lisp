...
(defun pq-insert-all (pq nodes key)
  ...
  (pq-sort-and-remove-duplicates	(append pq nodes)
													key))
...
(defun pq-sort-and-remove-duplicates (list key)
  ...
  (remove-duplicates	(stable-sort	(copy-list list) 
													#'< 
													:key key) 
								:key #'node-label 
								:from-end T))
...
(defun expand-node (current h explored)
  "current: the node-struct of the node to be expanded
   ...
   explored: the set of already explored nodes"
	(inList	(expand (node-label current))
				explored 
				h 
				current)
)

(defun inList (cities explored h current)
	(if (null (find (first cities) explored :key #'node-label))
		(if (null cities)
			nil
			(cons (make-node	:label (first cities) 
										:g (+	(get-distance (node-label current) (first cities)) 
													(node-g current))
										:h (funcall h (first cities))
										:parent current)
				(inList	(rest cities) 
							explored 
							h 
...
(defun search-step (goal h frontier explored)
  "goal: the label of the goal city
   ...
   explored: the set of already explored nodes"
   (if (equal	goal 
					(node-label (first frontier)))
		(list	goal	(cons (-first frontier) explored))
		(if (null frontier)
			(list nil explored)
			(search-step 
				goal
...
(sqrt 	(+ 	(expt	(-	(first (get-coordinates goal)) 
									(first (get-coordinates city))) 
								2) 
					(expt	(-	(second (get-coordinates goal))
									(second (get-coordinates city)))
								2 )))
)
...
(defun a*-search (start goal)
	...
	(if (or (null (expand start)) (null (expand goal)))
		nil
		(let (
			(r (search-step	goal																									;; R
									#'(lambda (x) (geo-heuristic x goal)) 
									(list (make-node	:label start
																:h (geo-heuristic start goal) 
																:parent nil)) 
									nil)))
			(labels (	
				(rev-path (path)  																									;; REV-PATH
					(if (null path) 
						nil 
						(cons	(node-label path)
									(rev-path (node-parent path)))))
									
				(get-visited-cities (list-of-paths) 																				;; GET-VISITED-CITIES
					(if (null list-of-paths) 
						nil
						(remove-duplicates	(append	(reverse (rev-path (first list-of-paths)))
																		(get-visited-cities (rest list-of-paths)))
														:from-end t))))
				(let (
					(path-to-goal (reverse (rev-path (first (second r)))))											;; PATH-TO-GOAL
					(path-visited-cities (get-visited-cities (reverse (second r))))									;; PATH-VISITED-CITIES
					(path-length (node-g (first (second r)))))																;; PATH-LENGTH
					
				(list path-length path-to-goal path-visited-cities))))))
;;
;; (e)
;;
...
(defun from-city (start)
	(labels (
		(get-cities (city-list) 																										;; GET-CITIES
			(let	((goal (first (first city-list))))
				(if (null city-list) 
					nil
					(let ((path-to-city (first (second (search-step	goal 												;; PATH-TO-CITY
																						#'(lambda (x) 0) 
																						(list (make-node	:label start
																													:parent nil))
																						nil)))))
						(cons	(list	(node-label path-to-city)
											(node-g path-to-city)) 
									(get-cities (rest city-list))))))))
		(sort	(get-cities *cities*) 
				#'< 
				:key #'second)))


;;TODO: What happens when goal not in list of cities. (like Tuebingen)
