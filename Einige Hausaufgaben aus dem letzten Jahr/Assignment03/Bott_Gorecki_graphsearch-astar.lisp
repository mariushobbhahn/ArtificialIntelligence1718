...
(let ((a (get-coordinates city))
	  (b (get-coordinates goal)))
...
; inserts node sorted by f-value into the queue
; argument node is a list of the nodename and its f_value
(defun Pqueue (node queue)
(if (< (cadr node) (car queue)) (append node queue)
(append (list(car queue)) (priority node (cdr queue)))))

; inserts list of elements into queue
(defun insertPqueue (queue elems)
(if (null elems) queue
(insertPqueue (Pqueue (car elems) queue) (cdr elems))))

; choses next node to expand
(defun toExpand (mindis succesors start)
(if (= mindis (get-distance start (car successors))) (list(car successors)(cdr successors))
(append (car successors (toExpand mindis (cdr successors) start)))))

; GIBT DISTANZEN AB START ZURÜCK
;(mapcar #'(lambda (x) (get-distance start x)) (expand start))

(defun a*-search (start goal h)
  "A*: Find a path from start to goal using the heuristic h"
    (a*-helper 0 start goal h start '())
  )
  
(defun a*-helper (g start goal h queue visited path)
(let ((successors (expand(car queue)))
(mindis (min(mapcar #'(lambda (x) (get-distance start x)) (expand start)))))
; return if start = goal
(if (= start goal) (list g path visited)
(a*-helper 
(+ g (get-distance (car(toExpand mindis successors start)) start))
(car (toExpand mindis successors start))
goal
h
(insertPqueue queue (cdr(toExpand mindis successors start)))
(append visited (list start))
(append path (car(toExpand mindis successors start)))))
...
; Loop over the list of cities, after every iteration, sort the city distance into the list of existing cities + distances to Hamburg
