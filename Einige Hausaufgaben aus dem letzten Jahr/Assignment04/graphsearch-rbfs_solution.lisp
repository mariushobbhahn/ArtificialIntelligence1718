...
(defconstant *posinf* 10000)

(defstruct (rbfs-node) city f g)

(defun update-node-f (n f)
  (make-rbfs-node :city (rbfs-node-city n) :f f :g (rbfs-node-g n)))

(defun insert-updated (n list)
  "insert node n into the list sorted by f-value"
  (cond ((null list) (list n))
        ((< (rbfs-node-f n) (rbfs-node-f (first list)))  (cons n list))
        (T (cons (first list) (insert-updated n (rest list))))))

(defun update-best (succs new-f)
  "updates the best node with a new f-value and sorts it into the correct position"
  (insert-updated (update-node-f (first succs) new-f) (rest succs)))

(defun generate-successors (n goal)
  "generates a list of successor nodes sorted by f-value"
  (sort
   (loop for s in (expand (rbfs-node-city n)) collect
        (let ((sg (+ (rbfs-node-g n) (get-distance (rbfs-node-city n) s))))
          (make-rbfs-node :city s
                          :f (max (+ sg (heuristic s goal)) (rbfs-node-f n))
                          :g sg)))
   #'<
   :key #'rbfs-node-f))

(defun examine-successors (succs f-limit n goal)
  "examins the list of successors to see whether a path can be found that is at most as long as the f-limit"
  (if (> (rbfs-node-f (first succs)) f-limit)
      (list 'fail (rbfs-node-f (first succs)))
      (let ((rec (rbfs-helper (first succs)
                              (if (null (rest succs)) f-limit
                                  (min f-limit (rbfs-node-f (second succs))))
                              goal)))
        (if (eq (first rec) 'fail)
            (examine-successors (update-best succs (second rec)) f-limit n goal)
            (list (first rec) (cons (rbfs-node-city n) (second rec)))))))

(defun rbfs-helper (n f-limit goal)
  "returns ('fail f-limit) or (goal-g path)"
  (setf numvisited (1+ numvisited))
  (if (eq (rbfs-node-city n) goal)
      (list (rbfs-node-g n) (list goal))
      (examine-successors (generate-successors n goal) f-limit n goal)))

(defvar numvisited) ;; too lazy to pass this through the recursive calls explicitly

(defun rbfs (start goal)
  (setf numvisited 0)
  (append (rbfs-helper (make-rbfs-node :city start :f (heuristic start goal) :g 0)
                       *posinf*
                       goal)
          (list numvisited)))
