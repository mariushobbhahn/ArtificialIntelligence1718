 ...
 * @param <S> The generic parameter determining the type of the game state
 */
public abstract class Game<M,S> {
	 ...
	 */
	public abstract Game<M,S> performMove(final M move);
	
	/**
	 * @return the internal game state, for use e.g. in a heuristic
	 */
	...
	public static <M,S> int play(
		final Game<M,S> game,
		final Agent<M,S> player1,
		final Agent<M,S> player2
	) {
		Game<M,S> g = game;
